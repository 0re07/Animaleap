//インクルードガード
	...
	float CalculateDiffuse(in float3 lightRay, in float3 normal)
	{
	    ...
	    float nDotL = saturate(dot(normal, lightRay));
		return sqrt(nDotL);
	...
	/// <summary>
	/// ハーフランバート拡散照明のライティング計算
	/// </summary>
	/// <param name="normal">法線(ビュー)</param>
	/// <param name="light">ライト構造体</param>
	float CalculateHalfLambertLight(in float3 normal, in Light light)
	{
    return saturate(dot(normal, -light.direction));

    float p = saturate(dot(normal, -light.direction));
		p = p * 0.5f + 0.5f;
		return p * p;
	}
