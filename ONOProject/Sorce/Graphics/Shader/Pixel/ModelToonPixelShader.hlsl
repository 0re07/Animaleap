...
SamplerState toonMapSampler : register(s2); // トゥーンマップテクスチャ
Texture2D toonMapTexture : register(t2); // トゥーンマップテクスチャ
...
PS_OUTPUT main(PS_INPUT input) : SV_TARGET
{
    ...
    //ハーフランバート拡散照明によるライティング
    float halfLambertDiff = 0;
    ...
    for (int i = 0; i < DX_D3D11_COMMON_CONST_LIGHT_NUM; i++)
    {
        ...
        halfLambertDiff += CalculateHalfLambertLight(normal.rgb, common.light[i]);
    }
    ...
    //トゥーンマップ
    //float4 toonCol = toonMapTexture.Sample(toonMapSampler, float2(halfLambertDiff, 0.0f));
    //result *= toonCol;
