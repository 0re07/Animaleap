...
InputManager::JOYPAD_IN_STATE InputManager::GetJPadInputState(JOYPAD_NO no)
{
	...
	case InputManager::JOYPAD_TYPE::XBOX_ONE:
	{
		...
		idx = static_cast<int>(JOYPAD_BTN::DPAD_UP);
		ret.ButtonsNew[idx] = x.Buttons[0];

		idx = static_cast<int>(JOYPAD_BTN::DPAD_DOWN);
		ret.ButtonsNew[idx] = x.Buttons[1];

		idx = static_cast<int>(JOYPAD_BTN::DPAD_LEFT);
...
const VECTOR& InputManager::GetPadLStickVec(void)
{

	// アナログパット情報取得
	auto joyPadState = GetJPadInputState(InputManager::JOYPAD_NO::PAD1);
	joyPadState.AKeyLX;	// 出力値（－1000～1000）左～右＋
	joyPadState.AKeyLY;	// 出力値（－1000～1000）上～下＋

	// ほしい値は－1～1なので1000で割る
	float padX = joyPadState.AKeyLX / 1000.0f;
	// 上下反転させる
	float padY = joyPadState.AKeyLY * -1.0f / 1000.0f;

	// Y方向なし入力ベクトル
	VECTOR padDir = { padX,0.0f,padY };
...
const VECTOR& InputManager::GetPadLStickVec(JOYPAD_NO no)
{
	// アナログパット情報取得
	auto joyPadState = GetJPadInputState(no);
	joyPadState.AKeyLX;	// 出力値（－1000～1000）左～右＋
	joyPadState.AKeyLY;	// 出力値（－1000～1000）上～下＋
