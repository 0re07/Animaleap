...
Quaternion Quaternion::Euler(double radX, double radY, double radZ)
{
    ...
    // 0(0)Å`2ÉŒ(360ìx)ÇÃîÕàÕÇ…é˚ÇﬂÇÈ
    auto RadIn2PI = [](double rad) {
...
Quaternion Quaternion::LookRotation(VECTOR dir, VECTOR up)
{
    ...
    dir = VNormalize(dir);
    VECTOR right = VNormalize(VCross(up, dir));
    up = VCross(dir, right);
    auto m00 = right.x;
    auto m01 = right.y;
    auto m02 = right.z;
...
VECTOR Quaternion::VNormalize(const VECTOR& vec)
{
    if (vec.x == 0.0f &&
        vec.y == 0.0f &&
        vec.z == 0.0f)
...
{
    auto SqrMagnitude = [](const VECTOR& v) {
        return v.x * v.x + v.y * v.y + v.z * v.z;
    };

    auto AngleDeg = [SqrMagnitude](const VECTOR& from, const VECTOR& to) {
	...
	double angle = AngleDeg(fromDir, toDir);
	if (angle >= 179.9196)
	{
        auto r = VCross(fromDir, {1.0f,0.0f,0.0f});
	...
	return Quaternion::AngleAxis(angle * (DX_PI / 180.0), axis);
